...
export default function ServicesHero() {
  ...
  return (
    ...
    >
      ...
      {/* Right side - Content */}
      <div className="relative z-10 flex w-full flex-col items-start justify-center px-4 md:w-1/2 md:pr-8 md:pl-16">
        ...
        <div className="relative mb-4 h-32 w-full max-w-xl md:mb-5 md:h-28">
          {servicesData.map((service, index) => (
            <div
              ...
              className="absolute inset-0"
            >
              <p className="w-full max-w-sm text-left font-pp-neue-montreal text-base text-white/80 md:text-lg">
                {service.description}
              </p>
            </div>
          ))}
          ...
          {servicesData.map((service, index) => (
            ...
            >
              <Link
                ...
                className="bg-secondary hover:bg-secondary/90 group relative z-20 inline-flex items-center gap-3 font-pp-neue-montreal text-sm text-white transition-all duration-300 md:text-base"
              >
                {service.cta.text}
                <ArrowRight
                  size={20}
