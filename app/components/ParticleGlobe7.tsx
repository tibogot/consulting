...
const LOCATIONS: GlobeLocation[] = [
  {
    ...
    color: 0x39ff14,
    description:
      "Main hub — project delivery, strategy, and coordination across markets.",
  },
  {
    name: "London",
    lon: -0.1276,
  ...
  },
  {
    ...
    color: 0xffff00,
    description:
      "Regional delivery and operational support across North Africa.",
  },
  {
    name: "Paris",
    lon: 2.3522,
...
export default function ParticleGlobe({
  ...
  const [listHoverLocation, setListHoverLocation] = useState<string | null>(
    null
  );
  const [globeHoverLocation, setGlobeHoverLocation] = useState<string | null>(
    null
  );
  ...
  const activeLocationName =
    selectedLocation ?? listHoverLocation ?? globeHoverLocation;
  ...
  useEffect(() => {
    ...
    const loadWorldMap = async (): Promise<ImageData> => {
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          ...
          const imageData = mapCtx.getImageData(
            0,
            0,
            mapCanvas.width,
            mapCanvas.height
        ...
        img.src =
          "/Equirectangular_projection_world_map_without_borders.svg.png";
      });
    };
    ...
    (async () => {
      ...
      const snapToLand = (
        ...
        for (let r = 1; r <= maxRadiusPx; r++) {
          ...
          for (let dx = -r; dx <= r; dx++) {
            ...
            const xBottom = x0 + dx;
            const yBottom = y0 + r;
            if (isLandPixel(xBottom, yBottom))
              return pixelToLonLat(xBottom, yBottom);
          }
          for (let dy = -r + 1; dy <= r - 1; dy++) {
            ...
            const xRight = x0 + r;
            const yRight = y0 + dy;
            if (isLandPixel(xRight, yRight))
              return pixelToLonLat(xRight, yRight);
          }
        }
      ...
      geometry.setAttribute(
        "originalPosition",
        new THREE.Float32BufferAttribute(originalPositionsArray, 3)
      );
      geometry.setAttribute(
        "color",
    ...
    `;
      ...
      interactionSphereGeometry = new THREE.SphereGeometry(
        globeRadius * 1.005,
        48,
        32
      );
      ...
      interactionSphere = new THREE.Mesh(
        interactionSphereGeometry,
        interactionSphereMaterial
      );
      ...
      locations.forEach((location) => {
        ...
        (
          markerGroup as THREE.Group & { animationData: MarkerAnimationData }
        ).animationData = {
          dot,
          halo,
      ...
      const animate = () => {
        ...
        if (locationMarkersRef.current) {
          ...
          for (let i = 0; i < markersLength; i++) {
            ...
            if (animData) {
              const markerName = (marker as THREE.Group).userData?.name as
                | string
                | undefined;
              const isActive =
                !!markerName && activeLocationNameRef.current === markerName;
              ...
              const haloOpacity = Math.min(
                0.55,
                haloOpacityBase * (isActive ? 2.2 : 1.0)
              );
              ...
              if (animData.dot.material instanceof THREE.MeshBasicMaterial) {
                const dotOpacity = Math.min(
                  1.0,
                  (0.9 + pulse * 0.06) * (isActive ? 1.15 : 1.0)
                );
        ...
        if (routesRef.current.length) {
          ...
          for (let i = 0; i < routesLength; i++) {
            ...
            const isActive =
              activeLocationNameRef.current !== null &&
              activeLocationNameRef.current === r.locName;
            const boost = isActive ? 1.75 : 1.0;
            r.material.opacity = Math.min(
              1.0,
              r.baseLineOpacity * fadeAlpha * boost
            );
            if (r.dot && r.dot.material instanceof THREE.SpriteMaterial) {
              r.dot.material.opacity = Math.min(
                1.0,
                r.baseDotOpacity * fadeAlpha * boost
              );
            }
        ...
        if (now - lastGlobeHoverUpdateRef.current > 100) {
          // Increased from 50ms to 100ms (10fps check)
            ...
            if (distSq < repulseRadiusWorld * repulseRadiusWorld) {
              ...
              if (frontDot > 0) {
                ...
                if (dist < repulseRadiusWorld && dist > 0.001) {
                  const force =
                    Math.pow(1 - dist / repulseRadiusWorld, 2) *
                    repulsionStrength;
  ...
  return (
    <div className={`relative h-full w-full ${className}`}>
      <div
        ref={containerRef}
        className={`absolute inset-0 h-full w-full overflow-hidden bg-transparent ${globeHoverLocation ? "cursor-pointer" : ""}`}
      />

      {(() => {
        const LocationsPanelBody = ({
          variant,
        }: {
            ...
            <div className="mt-4">
              {activeLocation ? (
                <>
                  ...
                  <div className="mt-3 text-[11px] tracking-[0.16em] text-white/40 uppercase">
        ...
        return (
          <>
            {/* Desktop: left-side locations list (drives the globe highlight) */}
            <div
              className="pointer-events-auto absolute top-4 left-4 z-10 hidden font-pp-neue-montreal text-white md:top-8 md:left-8 md:block"
            ...
            {/* Mobile: collapsible drawer so it doesn't cover the globe */}
            <div className="pointer-events-auto absolute right-4 bottom-4 left-4 z-10 font-pp-neue-montreal text-white md:hidden">
              <div className="overflow-hidden rounded-[2px] border border-white/10 bg-black/25 backdrop-blur-sm">
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="shrink-0 text-[11px] tracking-[0.16em] text-white/60 uppercase">
