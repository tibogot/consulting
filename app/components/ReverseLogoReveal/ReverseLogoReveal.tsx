...
interface ReverseLogoRevealProps {
  imageSrc?: string;
  imageSrc2?: string;
}

export default function ReverseLogoReveal({
  imageSrc = "/codegrid-images/hero-img-layer-1.jpg",
  imageSrc2 = "/codegrid-images/hero-img-layer-2.png",
}: ReverseLogoRevealProps) {
  ...
  useGSAP(
    () => {
      ...
      // Set initial scale to 1 (logo is normal size)
      const initialScale = 1;
      const finalScale = 15; // Scale large enough to reveal full image

      // Function to set mask transform - scales from center of viewport
      const setMaskScale = (scale: number) => {
        ...
        maskGroupRef.current.setAttribute(
          "transform",
          `translate(${centerX}, ${centerY}) scale(${scale}) translate(-100.5, -30.5)`
        );
      };

      // Set initial scale
      setMaskScale(initialScale);
      ...
      function setupScrollTrigger() {
        ...
        scrollTriggerInstance = ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            // Logo mask scales from 1 to finalScale
            const maskScale =
              initialScale + scrollProgress * (finalScale - initialScale);

            // Image container scales from 1x to 1.5x for parallax effect
            const heroImgContainerScale = 1 + scrollProgress * 0.5;

            setMaskScale(maskScale);

            gsap.set(heroImgContainerRef.current, {
              scale: heroImgContainerScale,
            });

            // Hide the container when mask is large enough (around 95% progress)
        ...
        // Force initial update to set the starting state
      ...
      setTimeout(() => {
        ...
        // Force scroll to top to ensure section is visible
        window.scrollTo(0, 0);
      }, 100);
      ...
      const handleResize = () => {
        if (scrollTriggerInstance) {
          ...
          const maskScale =
            initialScale + currentProgress * (finalScale - initialScale);
          setMaskScale(maskScale);
        }
        ScrollTrigger.refresh();
        setupScrollTrigger();
    ...
    { scope: heroRef, dependencies: [lenis] }
  );
  ...
  return (
    ...
    >
      {/* Hero Image Container - Full screen image */}
      <div
      ...
      {/* Black Overlay with Logo SVG Mask */}
      {/* The logo SVG is used as a mask - image is visible only inside the logo shape */}
      <div className="fixed top-0 left-0 w-screen h-screen z-[1] pointer-events-none bg-black">
        <svg
          width="100%"
          height="100%"
        ...
        >
          <defs>
            <mask id="logoMask">
              {/* White = shows overlay (black), Black = hides overlay (reveals image) */}
              <rect width="100%" height="100%" fill="white" />
              <g ref={maskGroupRef}>
                {/* Logo SVG - using the actual SVG content as mask */}
                <svg
                  x="0"
                  y="0"
                  viewBox="0 0 201 61"
          ...
          {/* Black rectangle that gets masked - logo shape reveals image */}
          <rect width="100%" height="100%" fill="black" mask="url(#logoMask)" />
        </svg>
      </div>

      {/* Visible Logo SVG - for reference/centering */}
      <div className="fixed top-0 left-0 w-screen h-screen flex items-center justify-center z-[2] pointer-events-none opacity-0">
