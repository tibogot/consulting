...
interface PinnedImageRevealProps {
  imageSrc: string;
  imageAlt?: string;
  useLogo?: boolean; // Use logo mask instead of circle
}
...
export default function PinnedImageReveal({
  ...
  useGSAP(
    () => {
      ...
      const scrollDistance = isMobile ? "200vh" : "300vh";

      // Initial circle size - smaller on mobile
      const initialRadiusPx = isMobile ? 100 : 150;

      // INITIAL STATE: Circle centered on screen
      const initialCenterXPercent = 50;
      const initialCenterYPercent = 50;
      ...
      if (useLogo) {
        ...
        if (imageContainer) {
          ...
          // Add click handler to test animation (will replace with scroll later)
          const handleClick = () => {
      ...
      if (!useLogo) {
        // Animate circle from initial size to full screen
        tl.to(
          imageWrapper,
          {
      ...
      // For logo mode, we're testing the final state first, no animation yet
    },
  ...
  return (
    ...
    >
      {/* Image wrapper - full size container for clip path or mask */}
      <div ref={imageWrapperRef} className="absolute inset-0">
        {/* Image container - masked with logo SVG */}
