...
function Card({ data }: { data: CardData }) {
  return (
    <div className="card relative w-full">
      <div className="card-inner bg-black text-white border-white/20 h-[600px] w-full overflow-hidden border-t px-4 py-4 md:h-[600px] md:px-8 md:py-6">
        <div className="flex h-full w-full flex-col md:flex-row">
          {/* Number */}
          <div className="flex w-full items-start md:w-1/12">
            <span className="text-sm leading-none font-pp-neue-montreal text-white/60">
              {data.number}
            </span>
          </div>
          ...
          <div className="flex w-full flex-col items-start md:w-5/12 md:pr-6">
            <div>
              <h3 className="-mt-1 text-4xl leading-none font-pp-neue-montreal  text-white">
                {data.title}
              </h3>
...
export default function StickyStackScroll() {
  ...
  useGSAP(
    () => {
      ...
      cards.forEach((card, index) => {
        ...
        const nextCard = !isLastCard ? cards[index + 1] : null;
        ...
        if (!isLastCard) {
          ...
          // Fade out animation - fade card as next card comes over it
  ...
  return (
    <div className="overflow-x-hidden" ref={container}>
      <section className="text-white bg-black intro mx-auto px-4 py-20 text-center md:px-8 md:py-30">
        <AnimatedText delay={0.0} stagger={0.3}>
          <h2 className="font-pp-neue-montreal mx-auto mb-6 max-w-4xl text-4xl md:text-5xl lg:text-6xl ">
      ...
      {/* Mobile Layout */}
      <section className="text-white block w-full py-8 md:hidden">
        <div className="space-y-8 px-4">
          {cardData.map((data) => (
            <div
              key={data.number}
              className="bg-black border-white/20 overflow-hidden border-t py-6"
            >
              <span className="text-sm leading-none font-pp-neue-montreal text-white/60">
                {data.number}
              </span>
              <h3 className="mt-4 text-4xl leading-none font-pp-neue-montreal  text-white">
                {data.title}
              </h3>
              <div className="mt-8" />
